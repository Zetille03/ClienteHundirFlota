...
public class ColocacionBarcosFrame extends JFrame{
	...
	MenuFrame menuFrame;
	ColocacionBarcosFrame thisFrame = this;
	//------------------------------
	...
	ColocacionBarcosFrame(String nombreUsuario, ObjectOutputStream salida,String contrincante, MenuFrame menuFrame,int id_partida)
	{
		...
		principal.setBorder(new EmptyBorder(15,5,0,25));
		
		//A�ADIR LISTENER AL TABLERO PARA QUE AL PULSAR R SE CAMBIE LA ROTACION
		
		//--CREAR TABLERO
		crearTablero();
		
		
		setTitle("Eleccion Barcos - "+nombreUsuario);
		...
		try {
			mensajeAyuda("Coloca primer barco");
			colocarBarco(this,3);
			mensajeAyuda("Coloca segundo barco");
			colocarBarco(this,3);
			mensajeAyuda("Coloca tercer barco");	
			colocarBarco(this,4);
			mensajeAyuda("Coloca cuarto barco");
			colocarBarco(this,4);
			mensajeAyuda("Coloca último barco");
			colocarBarco(this,5);
	...
	public void mensajeAyuda(String texto) {
		this.textoAyuda.setText(texto);
		this.textoAyuda.setForeground(Color.red);
	...
	public void crearTablero()
	{
		...
		for(int x = 0;x<100;x++)
		{
			...
			if(x%10==0 && x!=99)
			{
				...
				principal.add(boton[x]);
		}
		
		JPanel panelrotar = new JPanel();
		principal.add(panelrotar);
		...
		panelNorte.add(textoAyuda);
		JButton botonEnviar = new JButton("Enviar");
		botonEnviar.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				if(arrayBarcos.size()==5) {
					try {
						menuFrame.nuevaPartidaFrame.barcosJugador = arrayBarcos;
					...
					mensajeAyuda("Selecciona los 5 barcos");
				}
			}
		});
	...
	class EanadirBarco implements MouseListener
	{
		...
		public void mouseClicked(MouseEvent arg0) 
		{
			...
			if(es_posible_colocar)
			{
				ArrayList<Boton> botonesBarco = new ArrayList<Boton>();
				for(int x = 0; x<100;x++)
				{
...
//						boton[x].setText(""+contador_barco);
				...
				arrayBarcos.add(new Barco(botonesBarco));
				contador_barco++;
				terminarAnadirBarco();
	...
	public static void colocarBarco(ColocacionBarcosFrame frame, int n_barcos) throws InterruptedException
	{

				frame.anadirBarco(n_barcos);
				
				while(frame.proceso==1)
				{
					Thread.sleep(1);
				}
